...
void HitEffect::Draw(Camera& Cam)
{
	...
	if (!PIPELINE)
	{
		...
		//パーリンノイズ
		DISPLACEMENT_NOISE_TEX = NoiseGenerator::PerlinNoise2D("HitEffect - DisplacementNoiseTex", IMG_SIZE, 12, 6, 1.647f, 0.775f);
		ALPHA_NOISE_TEX = NoiseGenerator::PerlinNoise2D("HitEffect - AlphaNoiseTex", IMG_SIZE, 7, 2, 0.79f, 0.5f);
