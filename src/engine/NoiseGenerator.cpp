...
std::shared_ptr<TextureBuffer> NoiseGenerator::PerlinNoiseFractal(const Vec2<int>& Size, const int& Split, const int& Octaves, const float& Persistence)
{
	//最大分割数
	static const int SPLIT_MAX = 256;
	assert(0 < Split && Split <= SPLIT_MAX);
	...
	{
		//シェーダ
		auto cs = D3D12App::Instance()->CompileShader("resource/engine/PerlinNoiseFractal.hlsl", "CSmain", "cs_5_0");
		//ルートパラメータ
		std::vector<RootParam>rootParams =
		{
		...
		//定数バッファ生成
		CONST_BUFF = D3D12App::Instance()->GenerateConstantBuffer(sizeof(ConstData), 1, nullptr, "PerlinNoiseFractal - ConstantBuffer");

		//構造体バッファ生成
		STRUCTURED_BUFF = D3D12App::Instance()->GenerateStructuredBuffer(sizeof(Vec2<float>), pow(SPLIT_MAX + 1, 2), nullptr, "PerlinNoiseFractal - StructuredBuffer");
	}
	...
	//描き込み先用テクスチャバッファ生成
	auto result = D3D12App::Instance()->GenerateTextureBuffer(Size, DXGI_FORMAT_R32G32B32A32_FLOAT, ("PerlinNoiseFractal - " + std::to_string(ID++)).c_str());
