...
float KuroMath::GetSpline(const float& T, const int& P1Idx, const std::vector<float>& Array, bool Loop)
{
    ...
    if (p1_idx == 0)    //p1‚ªæ“ª
    {
        if (Loop)p0_idx = endIdx;   //ƒ‹[ƒv‚µ‚Ä‚é‚È‚çp0‚Í––”ö
        else p0_idx = p1_idx;
    }
    else
    {
        p0_idx = p1_idx - 1;    //ˆê‚Â‘O
    }

    if (p2_idx == endIdx)   //p2‚ª––”ö
    {
        if (Loop)p3_idx = 0;    //ƒ‹[ƒv‚µ‚Ä‚é‚È‚çæ“ª
        else p3_idx = p2_idx;
    }
